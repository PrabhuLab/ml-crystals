...
$(x\prime,y\prime,z\prime)$. It consists of a rotation/reflection matrix
...
w_{2} \\
w_{3}
\end{pmatrix}\quad\text{or}\quad{\mathbf{x}\mathbf{\prime}} = \mathbf{W}\mathbf{x} + \mathbf{w}$$
...
fractional coordinates $\left( x_{f1},y_{f1},z_{f1} \right)$ and
$\left( x_{f2},y_{f2},z_{f2} \right)$ requires the full crystallographic
distance formula:

$$d = \left( \begin{aligned}
 & {a^{2}\left( x_{f1} - x_{f2} \right)^{2} + b^{2}\left( y_{f1} - y_{f2} \right)^{2} + c^{2}\left( z_{f1} - z_{f2} \right)^{2}} \\
 & {+ 2ab\left( x_{f1} - x_{f2} \right)\left( y_{f1} - y_{f2} \right)\cos\gamma} \\
 & {+ 2ac\left( x_{f1} - x_{f2} \right)\left( z_{f1} - z_{f2} \right)\cos\beta} \\
 & {+ 2bc\left( y_{f1} - y_{f2} \right)\left( z_{f1} - z_{f2} \right)\cos\alpha}
...
The uncertainty in the distance, $\sigma_{d}$, is found by applying the
general formula to the distance $d$. The input parameters $p_{i}$ are
...
Where, for each unique atomic site *j* in the asymmetric unit: - $m_{j}$
is the **Wyckoff multiplicity** (how many equivalent atoms are generated
by symmetry). - $\text{occ}_{j}$ is the **site occupancy** (accounts for
...
represents the grand total sum of the lengths of every bond in the unit
cell. The denominator,
$\sum_{j}\left( m_{j} \cdot \text{occ}_{j} \cdot n_{j} \right)$,
represents the total number of bonds in the unit cell.
