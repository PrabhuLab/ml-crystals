...
extract_unit_cell_metrics <- function(cif_content) {
  ...
  # Double the lengths of the unit cell parameters
...
#   }
# 
#   # Expand coordinates by applying each symmetry operation and transformations of +1 and -1
...
#     transformations <- expand.grid(tx = c(-1, 0, 1), ty = c(-1, 0, 1), tz = c(-1, 0, 1))
# 
#     new_coords <- rbindlist(lapply(1:nrow(symmetry_operations), function(i) {
#       rbindlist(lapply(1:nrow(transformations), function(j) {
#         tx <- transformations[j, "tx"]
#         ty <- transformations[j, "ty"]
#         tz <- transformations[j, "tz"]
# 
#         new_x <- apply_operation(symmetry_operations[i, x], x + tx, y + ty, z + tz)
#         new_y <- apply_operation(symmetry_operations[i, y], x + tx, y + ty, z + tz)
#         new_z <- apply_operation(symmetry_operations[i, z], x + tx, y + ty, z + tz)
# 
#         data.table(
#           Label = paste(row$Label, i, j, sep = "_"), # Apply unique label
#           x_a = new_x %% 1,  # Apply modulus to keep coordinates within [0,1)
#           y_b = new_y %% 1,  # Apply modulus to keep coordinates within [0,1)
#           z_c = new_z %% 1  # Apply modulus to keep coordinates within [0,1)
#         )
#       }))
#     }))
# 
...
# 
#   # Remove duplicates
#   unique_coords <- transformed_coords %>%
#     distinct(x_a, y_b, z_c, .keep_all = TRUE)
# 
#   # unique_coords <- unique(unique_coords, by = c("x_a", "y_b", "z_c"))
# 
...
```{r}
calculate_distances <- function(atomic_coordinates, transformed_coords, unit_cell_metrics) {
  ...
  coords_transformed <- as.matrix(transformed_coords[, .(x_a, y_b, z_c)])
  labels_atomic <- atomic_coordinates$Label
  labels_transformed <- transformed_coords$Label

  # Calculate pairwise differences in coordinates between atomic and transformed coordinates
  delta_x <- outer(coords_atomic[, 1], coords_transformed[, 1], "-")
  delta_y <- outer(coords_atomic[, 2], coords_transformed[, 2], "-")
  delta_z <- outer(coords_atomic[, 3], coords_transformed[, 3], "-")

  # Debug: Check pairwise differences
  # cat("Pairwise differences (delta_x, delta_y, delta_z):\n")
  # print(delta_x)
  # print(delta_y)
  # print(delta_z)
  ...
  # Debug: Check differences after PBC application
  # cat("After PBC application (delta_x, delta_y, delta_z):\n")
  # print(delta_x)
  # print(delta_y)
  # print(delta_z)
  ...
  # # Debug: Check squared distances
  # cat("Squared distances (r2):\n")
  # print(head(r2))
  ...
  atom_pairs <- expand.grid(Atom1 = labels_atomic, Atom2 = labels_transformed, KEEP.OUT.ATTRS = TRUE)
...
for (cif in cif_tab) {
  ...
  distances <- calculate_distances(atomic_coordinates, transformed_coords, unit_cell_metrics)
