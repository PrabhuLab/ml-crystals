...
calculate_distances <- function(transformed_coordinates) {
  coords <- as.matrix(transformed_coordinates[, c('x_a', 'y_b', 'z_c')])
  n <- nrow(coords)
  
  # Calculate the distance matrix using matrix operations
  distance_matrix <- sqrt(rowSums((coords[row(coords), ] - coords[col(coords), ])^2))
  distance_matrix <- matrix(distance_matrix, n, n)
  
  # Create a data frame for the distance matrix
...
time_read_all <- 0
time_extract_formula_all <- 0
time_extract_structure_all <- 0
time_extract_space_group_name_all <- 0
time_extract_space_group_number_all <- 0
time_extract_unit_cell_all <- 0
time_extract_atomic_all <- 0
time_extract_symmetry_all <- 0
time_apply_symmetry_all <- 0
time_calculate_distances_all <- 0

# List CIF files
time_cif_list <- system.time({
  cif_list <- Sys.glob(paths = "data/*.cif")
  cif_tab <- lapply(cif_list, readLines, warn = FALSE)
})
cat("Time to list CIF files:", time_cif_list["elapsed"], "seconds\n")

# Extracting from all the .cifs:
time_extraction <- system.time({
  ...
  for (cif_file in cif_list) {
    ...
    time_read_all <- time_read_all + time_read["elapsed"]

    # Extract the compound name from the file name
    compound_name <- tools::file_path_sans_ext(basename(cif_file))

    # Extract the relevant data
    time_extract_formula <- system.time({
      chemical_formula <- extract_chemical_formula(cif_content)
    })
    time_extract_formula_all <- time_extract_formula_all + time_extract_formula["elapsed"]

    time_extract_structure <- system.time({
      structure_type <- extract_structure_type(cif_content)
    })
    time_extract_structure_all <- time_extract_structure_all + time_extract_structure["elapsed"]

    time_extract_space_group_name <- system.time({
      space_group_name <- extract_space_group_name(cif_content)
    })
    time_extract_space_group_name_all <- time_extract_space_group_name_all + time_extract_space_group_name["elapsed"]

    time_extract_space_group_number <- system.time({
      space_group_number <- extract_space_group_number(cif_content)
    })
    time_extract_space_group_number_all <- time_extract_space_group_number_all + time_extract_space_group_number["elapsed"]

    time_extract_unit_cell <- system.time({
      unit_cell_metrics <- extract_unit_cell_metrics(cif_content)
    })
    time_extract_unit_cell_all <- time_extract_unit_cell_all + time_extract_unit_cell["elapsed"]

    time_extract_atomic <- system.time({
      atomic_coordinates <- extract_atomic_coordinates(cif_content)
    })
    time_extract_atomic_all <- time_extract_atomic_all + time_extract_atomic["elapsed"]

    time_extract_symmetry <- system.time({
      symmetry_operations <- extract_symmetry_operations(cif_content)
    })
    time_extract_symmetry_all <- time_extract_symmetry_all + time_extract_symmetry["elapsed"]

    # Apply symmetry operations to atomic coordinates
    time_apply_symmetry <- system.time({
      transformed_coordinates <- apply_symmetry_operations(atomic_coordinates, symmetry_operations)
    })
    time_apply_symmetry_all <- time_apply_symmetry_all + time_apply_symmetry["elapsed"]

    # Calculate distances between atoms
    time_calculate_distances <- system.time({
      distance_df <- calculate_distances(transformed_coordinates)
    })
    time_calculate_distances_all <- time_calculate_distances_all + time_calculate_distances["elapsed"]
...
cat("Total time for extracting from all CIF files:", time_extraction["elapsed"], "seconds\n")

# Combine all results into a single data frame
time_combine <- system.time({
  results_df <- do.call(rbind, results)
})

cat("Time to combine results into a data frame:", time_combine["elapsed"], "seconds\n")

# Print the accumulated times for each function
cat("Total time to read CIF files:", time_read_all, "seconds\n")
cat("Total time to extract chemical formula:", time_extract_formula_all, "seconds\n")
cat("Total time to extract structure type:", time_extract_structure_all, "seconds\n")
cat("Total time to extract space group name:", time_extract_space_group_name_all, "seconds\n")
cat("Total time to extract space group number:", time_extract_space_group_number_all, "seconds\n")
cat("Total time to extract unit cell metrics:", time_extract_unit_cell_all, "seconds\n")
cat("Total time to extract atomic coordinates:", time_extract_atomic_all, "seconds\n")
cat("Total time to extract symmetry operations:", time_extract_symmetry_all, "seconds\n")
cat("Total time to apply symmetry operations:", time_apply_symmetry_all, "seconds\n")
cat("Total time to calculate distances:", time_calculate_distances_all, "seconds\n")
