...
calculate_distances <- function(transformed_coords, unit_cell_metrics) {
  ...
  atom_pairs <- expand.grid(Atom1 = labels, Atom2 = labels, KEEP.OUT.ATTRS = TRUE)
  ...
  # Filter out self-distances and distances above 10 Angstrom
  distances <- distances[Atom1 != Atom2 & Distance > 0 & Distance < 10]
  
  # Sort each row to ensure Atom1 is always less than or equal to Atom2 and remove duplicates
  #try text based sorting
  # sorted_pairs <- t(apply(distances[, .(Atom1, Atom2)], 1, sort))
  # distances[, `:=`(Atom1 = sorted_pairs[, 1], Atom2 = sorted_pairs[, 2])]
...
minimum_distance <- function(distances, delta = 0.1) {
  ...
  bonded_pairs <- distances[dmin, on = .(Atom1), allow.cartesian = TRUE][Distance <= dcut, .(Atom1, Atom2, dcut)]
  
  return(unique(bonded_pairs))
...
```{r}
hoppe <- function(distances, delta = 0.5) {
  ...
  for (atom in unique_atoms) {
    ...
    while (TRUE) {
      ...
      if (abs(davg - prev_davg) <= 0.001) break
    }
    
    # Step 4: Determine bonded pairs based on the tolerance parameter δ
    bonded <- atom_distances[exp(1 - (Distance / davg)^6) >= delta]
