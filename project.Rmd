...
calculate_distances <- function(transformed_coords, unit_cell_metrics) {
  ...
  # Filter out zero distances (self-distances) and distances above 10 Ångström
  distances <- distances[Atom1 != Atom2 & Distance > 0 & Distance < 10]
  
  # Filter out duplicates
  distances <- distances[!duplicated(distances)]
  
  return(distances)
}

# Function to calculate bonds using the Minimum Distance Method
calculate_bonds_minimum_distance <- function(distances, delta = 0.1) {
  # Find the minimum distance for each atom
  min_distances <- distances[, .(min_distance = min(Distance)), by = Atom1]
  
  # Define the cutoff distance
  min_distances[, d_cut := (1 + delta) * min_distance]
  
  # Determine bonded pairs based on the cutoff distance
  bonded_pairs <- merge(distances, min_distances, by = "Atom1")
  bonded_pairs <- bonded_pairs[Distance <= d_cut, .(Atom1, Atom2, Distance)]
  
  return(bonded_pairs)
}

# Function to calculate bonds using Brunner's Largest Reciprocal Gap Method
calculate_bonds_brunner <- function(distances, delta = 0.0001) {
  # Find the reciprocal gaps for each atom
  distances[, reciprocal_gap := 1/Distance - shift(1/Distance, type = "lead"), by = Atom1]
  
  # Determine the largest reciprocal gap
  max_reciprocal_gap <- distances[, .SD[which.max(reciprocal_gap)], by = Atom1]
  
  # Define the cutoff distance
  max_reciprocal_gap[, d_cut := Distance + delta]
  
  # Determine bonded pairs based on the cutoff distance
  bonded_pairs <- merge(distances, max_reciprocal_gap[, .(Atom1, d_cut)], by = "Atom1")
  bonded_pairs <- bonded_pairs[Distance <= d_cut, .(Atom1, Atom2, Distance)]
  
  return(bonded_pairs)
}

# Function to calculate bonds using Hoppe's Effective Coordination Numbers Method
calculate_bonds_econ <- function(distances, tolerance = 0.5) {
  # Helper function to calculate weighted average bond length iteratively
  calculate_weighted_average <- function(distances) {
    davg_prev <- min(distances)
    repeat {
      weights <- exp(1 - (distances / davg_prev)^6)
      davg_new <- sum(distances * weights) / sum(weights)
      if (abs(davg_new - davg_prev) <= 0.001) break
      davg_prev <- davg_new
    }
    return(davg_new)
  }
  
  # Find the weighted average bond length for each atom
  avg_distances <- distances[, .(d_avg = calculate_weighted_average(Distance)), by = Atom1]
  
  # Determine bonded pairs based on the exponential cutoff
  distances <- merge(distances, avg_distances, by = "Atom1")
  distances[, bonded := exp(1 - (Distance / d_avg)^6) >= tolerance]
  
  bonded_pairs <- distances[bonded == TRUE, .(Atom1, Atom2, Distance)]
...
for (cif in cif_tab) {
  ...
  minimum_bonds <- calculate_bonds_minimum_distance(distances)
  brunner_bonds <- calculate_bonds_brunner(distances)
  econ_bonds <- calculate_bonds_econ(distances)
  ...
  main_table <- rbindlist(list(main_table, data.table(
    ...
    minimum_bonds = list(minimum_bonds),
    brunner_bonds = list(brunner_bonds),
    econ_bonds = list(econ_bonds)
  )), fill = TRUE)
}
