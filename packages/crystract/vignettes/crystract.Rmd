...
$\gamma$). Their experimental uncertainties are also extracted.
...
complete set of atoms within the primary unit cell. Note that it requires the unit cell metrics to resolve distance tolerances across boundaries.
...
**coordination number (CN)**. `crystract` implements several robust, geometric, and mathematical approaches:
...
-   **`crystal_nn`**: A sophisticated approach combining Voronoi solid angles, ionic radii distance cutoffs, and electronegativity weights. *Note: This is a ground-up rebuild of Pymatgen's `CrystalNN` logic and is currently in active testing.*
