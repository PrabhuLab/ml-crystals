...
### 2.1. Obtaining the Required CIF Files

Due to licensing restrictions, the large dataset of clathrate CIF files from the ICSD is not bundled with the package. To reproduce this advanced workflow, you must download the required data yourself.

1.  **Find Required Files**: A complete list of all necessary ICSD entry codes and the structural category (subdirectory) they belong to can be found in the `required_ICSD_files.csv` file at the root of the `ml-crystals` repository.

2.  **Download Data**: Using a licensed copy of the [ICSD database](https://icsd.fiz-karlsruhe.de/), download the `.cif` file for each entry listed in the CSV.

3.  **Recreate the Directory Structure**: This is a critical step.
    *   Create a main directory on your computer, for example, `C:/my_clathrate_data/`.
    *   Inside this main directory, create the subdirectories exactly as named in the `category` column of the CSV file (e.g., `6c-16i-24k`, `2a-6d`, etc.).
    *   Place each downloaded `.cif` file into its corresponding category subdirectory.

4.  **Update Path in the Script**: In the code chunk below, you must change the `cif_root_dir` variable to point to the main directory you created (e.g., `C:/my_clathrate_data/`).

**To run the analysis yourself, you must also remove the `eval=TRUE` option from the R code chunk headers in the following sections.**

### 2.2 Loading the Bundled Example Data
...
```{r load-files, echo=TRUE, eval=TRUE}
# IMPORTANT: Update this path to point to your main data directory
cif_root_dir <- "/Users/donngo/repos/ml-crystals/packages/crystract/inst/extdata/Verified"
...
```{r run-batch-analysis-optimized, echo=TRUE, eval=TRUE}
...
```{r process-data, echo=TRUE, eval=TRUE}
# --- Timing Start ---
...
```{r show-removed-distances, echo=TRUE, eval=TRUE}
...
```{r generate-plot, echo=TRUE, eval=TRUE}
...
```{r show-final-data-table, echo=TRUE, eval=TRUE}
if (exists("plot_data") && nrow(plot_data) > 0) {
  datatable(
    plot_data,
