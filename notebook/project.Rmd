...
```{r}
setwd("/Users/donngo/repos/ml-crystals")
```
...
```{r}
# Get a list of all file paths ending in .cif from the data directory.
cif_file_paths <- Sys.glob(paths = "data/*.cif")

# Create a named list. The names will be the simple filenames (e.g., "my_crystal.cif"),
# and the values will be the contents of each file, read into a data.table.
names(cif_file_paths) <- basename(cif_file_paths)
cif_tabs <- lapply(cif_file_paths, fread, sep = "\n", header = FALSE)
```
...
propagate_angle_error <- function(bond_angles,
  ...
  work_dt <- work_dt[mag_a > 1e-9 &
                       mag_b > 1e-9] # Filter out zero-length vectors
...
# Iterate over the named list of CIF contents
for (file_name in names(cif_tabs)) {
  cif <- cif_tabs[[file_name]]
  ...
  # Check for NULL returns from extraction before proceeding
  if (is.null(atomic_coordinates) || is.null(symmetry_operations)) {
    warning(paste("Skipping file due to missing critical data:", file_name))
